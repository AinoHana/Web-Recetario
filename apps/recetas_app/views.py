...
from apps.usuarios.models import is_admin
...
def recetas_populares(request):
    ...
    if order_by == 'populares':
        if direction == 'asc':
            ...
            recetas_query = recetas_query.order_by('-num_favoritos')
    # Paginación
    paginator = Paginator(recetas_query, 12) # Mostrar 12 recetas por página
    ...
    context = {
        'recetas_populares': page_obj.object_list,
        'page_obj': page_obj,
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        ...
        html_pagination = render_to_string(
            'recetas_app/partials/pagination_controls.html',
            context,
            request=request
        )
        return JsonResponse({
...
# Vista para la búsqueda avanzada (original, no consolidada)
...
def crear_categoria(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('recetas_app:admin_options')
        else:
...
def admin_recetas_ajax(request):
    recetas = Receta.objects.all().order_by('-fecha_publicacion')
    context = {'recetas': recetas}
    html_content = render_to_string('recetas_app/partials/admin_recetas_list.html', context, request=request)
...
def admin_categorias_ajax(request):
    ...
    html_content = render_to_string('recetas_app/partials/admin_categorias_list.html', context, request=request)
    return JsonResponse({'html_content': html_content})
...
def administrar_usuarios(request):
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        html_content = render_to_string('recetas_app/partials/admin_usuarios_list.html', context, request=request)
        return JsonResponse({'html_content': html_content})
    return render(request, 'recetas_app/admin_options.html', context)
...
def editar_usuario(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, f'El usuario {usuario.username} ha sido actualizado.')
            return redirect('recetas_app:administrar_usuarios')
    else:
...
def contacto(request):
    from apps.usuarios.forms import ContactoForm
    if request.method == 'POST':
...
def inbox(request):
    ...
    return render(request, 'recetas_app/inbox.html', context)
